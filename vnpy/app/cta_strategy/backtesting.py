...
from typing import Callable  # Callable 是可以代入其他函数成为参数的函数
...
class BacktestingEngine:
    ...
    gateway_name = "BACKTESTING"  # 用于日志显示
    ...
    def __init__(self):
        ...
        self.start = None
        self.end = None
        self.rate = 0  # 手续费费率，按照成交金额计算费率，但是有一部分合约按照成交手数来计算费率e.g黄金
        self.slippage = 0  # 滑点  rate 和 slippage 是计算为一笔成交付出多少开销的关键参数
        self.size = 1  # 合约乘数
        self.pricetick = 0  # 最小的价格变动 会变动
    ...
    def clear_data(self):
        """
        Clear all data of last backtesting. 把很多self.变量设为空或无或0
        """
        self.strategy = None
        self.tick = None
        self.bar = None
    ...
    def set_parameters(
        ...
        """作用是设置回测的参数；把参数赋值定义给各种self.变量，比如self.mode = mode"""
        self.mode = mode
    ...
    def load_data(self):
        """ 加载数据。首先处理回测开始时间和结束时间；其次进行一个大循
            环：if self.mode == BacktestingMode.BAR: data = load_bar_data，调用
            load_bar_data或者load_tick_data函数；然后self.history_data.extend(data)，即列表
            的增加功能；输出加载进度，重设start和end。最后output历史数据加载完成。"""
    ...
    def calculate_result(self):
        ...
        # Add trade data into daily reuslt.  把交易数据添加到每日结果 * trades是字典
...
def optimize(
    ...
    # kate: 输出每次迭代的参数值
    print(setting, "start backtesting")
    ...
    # kate: 输出一次迭代完成
    print(setting, "finish backtesting")
